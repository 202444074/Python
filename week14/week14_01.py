...
def gen_rettime():
    return dt.now() - datetime.timedelta(hours=random.randint(0, 10)
                                         , minutes=random.randint(0,60)) # timedelta 안에 들어갈 수 있는 5가지 알아두기
def gen_rettime():
    return dt.now() + datetime.timedelta(hours=random.randint(0, 48)
